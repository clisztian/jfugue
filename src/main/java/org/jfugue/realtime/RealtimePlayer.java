...
public class RealtimePlayer 
{
	...
	public RealtimePlayer(Receiver receiver) throws MidiUnavailableException {
	...
	public void startNote(Note note) throws InvalidMidiDataException {
		
		
		if(receiver != null) {
			playNote(this.currentChannel, note.getValue(), note.getOnVelocity());
		}
		else {
    ...
    public void stopNote(Note note) throws InvalidMidiDataException {
    	...
    	if(receiver != null) {
			stopNote(this.currentChannel, note.getValue(), note.getOnVelocity());
		}
    	else {
    ...
    public void startChord(Chord chord) throws InvalidMidiDataException {
        for (Note note : chord.getNotes()) {
            startNote(note);
        }
    }

    public void stopChord(Chord chord) throws InvalidMidiDataException {
        for (Note note : chord.getNotes()) {
            stopNote(note);
        }
    }
	...
	public void playNote(int channel, int key, int velocity) throws InvalidMidiDataException {
		
		ShortMessage myMsg = new ShortMessage();
		myMsg.setMessage(ShortMessage.NOTE_ON, channel, key, velocity);
		long timeStamp = -1;
		receiver.send(myMsg, timeStamp);		
	}

	public void stopNote(int channel,int key, int velocity) throws InvalidMidiDataException {
		
		ShortMessage myMsg = new ShortMessage();
		myMsg.setMessage(ShortMessage.NOTE_OFF, channel, key, velocity);
		long timeStamp = -1;
		receiver.send(myMsg, timeStamp);		
	}	
